...
**쉬운 풀이**: 건축에 *건축 설계 패턴*이 있는 것과 같다. 집을 지을 때 *바퀴를 새로 발명하지 않는다* — *거실은 현관 근처에, 침실은 거실에서 떨어진 자리에, 욕실은 침실 근처*라는 *반복적으로 검증된 패턴*을 따른다. 온톨로지도 — *여러 도메인에서 반복적으로 등장하는 모양*이 있다. 그 모양을 *명시적으로 적어둔 것*이 ODP. 새 스키마를 짤 때 *이 ODP에서 골라 쓰면* — 시행착오가 줄어든다.
...
이 책 2부의 *결정적 순간*. 4개 N항 관계가 *드론 군집비행의 모양*을 데이터 모델로 담는다.

1976년 Peter Chen이 *Entity-Relationship Model*을 발표했을 때, 그는 *관계가 두 자리만 묶는다*는 자리에 머물지 않았다 — 원 논문에서 이미 *n-ary relationship*을 명시적으로 다뤘다. 그러나 이후 *관계형 데이터베이스*가 산업의 주류가 되면서, *2자리 관계*가 *기본 모양*이 되고 *N항*은 *외래키 테이블의 우회*로 흐려졌다.

그 흐려진 자리에서 *진짜 의미가 잘리는* 도메인이 있다. *드론이 임무에서 leader 자리에 들어가 17:30부터 19:00까지 일했다* — 이건 *4자리 관계 + 시점 2개*의 한 사실이다. 이걸 *4개의 외래키 테이블*로 흩어 적으면 — 데이터는 들어가지만 *사실의 단위*가 사라진다. 분석 시 *조인*으로 다시 모아야 하고, *제약*은 코드로 강제해야 한다.

이 절이 짚는 자리: *N항 관계가 일급 시민일 때 도메인이 자연스럽게 표현되는 모양*을 4가지 사례로 본다.
...
*같은 6개 어휘가 entity·관계·속성의 세 자리로 분산*. 도메인의 *의미*가 *데이터 모델의 어느 곳에서 표현되는가*는 — *작동 방식*이 결정한다.

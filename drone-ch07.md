...
*분석가가 같은 쿼리를 여섯 번째 짜는 자리에서 — 의자를 뒤로 민다.*

이 책 7장의 자리가 거기다. 6장에서 데이터를 채운 직후, 분석가는 *바로 쿼리를 짜기 시작한다*. 처음 몇 개의 쿼리는 단순하다 — *야간 광학 가능 드론*, *현재 리더*, *DRN-005와 통신 가능한 드론들*. 모두 *match-fetch*의 짧은 패턴.

그런데 — 같은 *패턴의 쿼리*가 반복해서 나타나는 자리가 보인다. *능력 등급 N 이상 보유 드론들* 패턴이 여섯 번 반복된다. *변수만 바꿔서* 복사·붙여넣기하던 분석가가 손을 멈춘다. *이거 — 함수로 만들어야 한다*.

그 순간이 — *재사용 가능한 추론의 단위*가 태어나는 자리다. *함수*. 이 장에서 그 함수 *네 개*를 짠다. 4장에서 짚었던 *Competency Questions 12개*의 절반(CQ 1·2·4·7·8·11)을 — 이 네 함수가 답한다.

이 장은 단순히 *함수를 정의하는 자리*가 아니다. 각 함수의 *3~5가지 사용 사례*, *성능 특성*, *대안 구현*, 그리고 *5가지 합성 패턴*까지 — *진짜 함수를 짜는 사고법*이 본진이다. 그리고 — *세 함수가 합성될 때* 8장의 클라이맥스 자리가 어떻게 자연스럽게 풀려나가는지를, 이 장이 *미리 준비한다*.

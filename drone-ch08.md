...
이 장이 책의 *클라이맥스*다. 7장에서 짠 네 함수가 *동시에 호출*되어, *한 사건의 영향*이 시스템 안에서 자동으로 펼쳐진다.

이 장은 *네 가지 시나리오*를 본다.
- **시나리오 A**: 리더 단일 손실 (이 장의 본진)
- **시나리오 B**: 두 드론 동시 손실
- **시나리오 C**: 통신 폭주
- **시나리오 D**: 임무 우선순위 변경

각 시나리오의 *5단계 시간축*과 *시스템이 답하는 자리·인간이 결정하는 자리*까지.

이론적 자리: *Forward chaining vs Backward chaining*, *Truth maintenance*, *사건 우선순위 결정*.

---

## 8.1 사건 매듭의 일반 패턴

본격 시나리오 전에 — 사건을 *어떻게 데이터 모델로 표현하는가*의 일반 패턴.
...
| 시점 | 단계 | 시스템 작업 | 인간 작업 |
|---|---|---|---|
| **T+0초** | 사건 감지 | drone_loss 매듭 입력 (자동 또는 수동) | — |
| **T+10초** | 영향 식별 | 8.2.3의 4가지 질문 자동 호출 | 알림 수신 |
| **T+30초** | 후보 도출 | 7장 함수 합성으로 대체 후보 자동 | — |
| **T+1분** | 분석가 판단 | 후보 명단 제시 | *재할당 결정* |
| **T+5분** | 재할당 완료 | 데이터 갱신, 새 상태 propagation | 확인·승인 |

이 시간축이 *실시간 시스템*에서는 더 짧다 (T+1초 이하). 책의 단순화에서는 *분석가의 판단 자리*가 살짝 늘려서 잡힘.

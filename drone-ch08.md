...
여기까지 — 도메인을 직시했고(4장), 스키마를 지었고(5장), 12대 드론을 데이터로 채웠고(6장), 네 함수를 손에 들었다(7장). 이제 *그 모든 것이 한 사건 앞에서 작동하는 순간*이다.
...
이것은 *기적*이 아니다. *제대로 짠 온톨로지에서 자연스럽게 나오는 결과*다. 분류 트리가 검색의 단위를 주고, N항 관계가 사건의 매듭을 잡고, 역할이 시점별 위치를 추적하고, 함수가 추론을 합성한다 — *네 도구가 동시에 일하는* 순간. 그것을 *눈으로 보는 것*이 이 장의 목적이다.
...
각 시나리오의 *5단계 시간축*과 — *시스템이 답하는 영역·인간이 결정하는 영역*까지. 이것도 이 책의 중요한 정직함이다. *모든 것을 자동화하지 않는다*. 답할 수 있는 것은 답하고, 판단해야 하는 것은 *판단할 수 있는 사람*에게 *깨끗이 정리해서 넘긴다*. 그것이 *제대로 짠 시스템의 모양*이다.

이론적 토대: *Forward chaining vs Backward chaining*, *Truth maintenance*, *사건 우선순위 결정* — 8장 후반의 ◇ 절에서 다룬다.

---

## 8.1 사건 매듭의 일반 패턴

본격 시나리오로 들어가기 전에 — 사건 자체를 *어떻게 데이터로 적는가*를 짚는다. 여기가 가볍게 보이지만, *온톨로지 작업에서 가장 자주 빠지는 함정*이 여기다.

흔한 첫 번째 시도: *사건을 attribute로 적는다*. "DRN-005가 손실됐다" → drone에 `lost: true` 속성. 이게 작동하는 경우가 있긴 하지만 — *언제 손실됐는지, 왜 손실됐는지, 무엇에 영향을 미쳤는지*를 같이 적어야 하는 순간, 속성 한두 개로는 부족해진다.

두 번째 시도: *사건을 entity로 둔다*. `loss_event isa event, owns time, owns cause`. 한 발 나아갔지만 — *사건이 무엇과 묶이는가*가 여전히 미해결. 별도 관계로 풀어야 한다.

*세 번째 방식*, 그리고 *진짜 모양*: **사건 자체가 N항 관계**다. *손실*이라는 사건은 — *한 드론*, *영향받은 임무들*, *깨진 링크들*, *시점*, *원인*을 *한 매듭으로 묶는* 것이다. 이것이 *Reified Event Pattern*이다. 5장에서 ODP로 짚은 그 패턴.

Davidson이 1967년에 본 *사건 자체가 존재자*라는 통찰이 — *데이터 모델로 옮겨진 사실*이다. 그리고 여기서야 비로소 — *한 사건이 다른 사건의 매듭에 들어가는* 일이 자연스러워진다. 8.2에서 *drone_loss 안에 communicates_with 관계 인스턴스가 들어간다*. 사건이 사건을 가리킨다. RDF에서는 *reification of reification*이라는 이중 우회가 필요한 부분이 — TypeDB에서 한 줄로 풀린다.
...
- 7.2의 `leader_candidates(m1)` — 임무 m1의 leader 자리에 *현재 들어가지 않은* 능력 4+ 드론들
...
여기까지 — *시스템이 답하는 영역*이다. 지금부터는 *분석가*. 화면의 후보가 자동으로 *재할당되지는 않는다*. 12초 동안, 분석가는 다음 것들을 *눈으로 확인*한다:
...
5분 28초. 한 드론이 손실되고 — 군집이 *복구되기까지*의 시간. 사람이 *손으로* 같은 일을 한다면 — 베테랑 분석가도 *15분 이상* 걸린다. 능력 트리에서 검색하고, 통신 표를 보고, 후보를 비교하고. 그 *시간 단축*이 — 이 책이 짠 시스템의 *진짜 가치*다.
...
이 시간축이 *실시간 시스템*에서는 더 짧다 (T+1초 이하). 책의 단순화에서는 *분석가의 판단*이 살짝 늘려서 잡혔다. 그 *판단*은 — *기계가 답할 수 없는 부분*이기 때문에 — *절대 줄어들면 안 되는 부분*이기도 하다.
...
*장면*. 2026년 4월 1일, 17시 23분 47초. 산악 지역 상공에 *돌풍 경보*가 뜬 지 12분 후. 분석가의 화면 — *왼쪽 모니터의 DRN-001 텔레메트리*와 *오른쪽 모니터의 DRN-009 텔레메트리*가 *동시에* 평선으로 떨어진다. 0.3초 간격. 사실상 *동시*.

분석가는 한 호흡 멈춘다. 이런 패턴을 처음 보는 게 아니다. 작년 동해 훈련에서, *돌풍에 두 대가 충돌*해 같이 추락한 적이 있었다. 그때 *복구까지 22분*. 그 22분 동안 — 다른 한 명의 분석가가 *손으로* 영향 분석을 하고 있었다.

시나리오 A보다 무거운 사건이다. 시나리오 A에서는 *후보 1대*가 있었다. 여기서는 — *후보 1대 + 중계 1대 손실*이다. 메시 네트워크가 *둘로 갈라질 위험*이 처음으로 등장한다.
...
*장면*. 17시 35분 18초. 시나리오 A의 재할당이 끝난 지 *7분 16초* 후. 새 리더 DRN-002가 막 안정화된 상태였다.

그리고 — *간섭 경보*가 뜬다. 산악 지역 상공 *3000~5000m*. 군용 항공기가 정기 노선으로 통과 중이라는 항공교통관제(ATC)의 사전 통보. 12분 전에 들어온 정보였지만 — *경로가 약간 어긋났다*. 4개 링크의 *link_quality*가 *0.85에서 0.25 아래*로 떨어진다. 1.8초 안에 모두.

분석가의 화면에 *세 가지가 동시에 들어온다*: 텔레메트리 품질 알림(노란색), 메시 라우팅 경고(주황색), ATC의 *15분 통과 예상*(파란색 정보). 드론은 살아 있다 — *모두 살아 있다*. 그러나 *네트워크의 신뢰성*이 *임무 수행 임계점 아래*로 떨어졌다. 이걸 어떻게 풀 것인가.
...
이 시나리오가 — *링크를 일급 시민으로 취급한 5장의 설계*가 *진짜로 빛을 발하는 순간*이다. 사건이 link 객체에 *plays*로 묶인다.
...
이 시나리오가 — *5장의 *깊은 설계*를 한 번 더 짚는 부분이다. 다시 보자:
...
이 한 줄. *관계가 다른 관계의 자리에 들어간다*. RDF에서는 — *각 link를 reification으로 노드화*하고, 다시 *그 노드를 disruption 노드와 연결*하는 *이중 우회*가 필요한 부분. TypeDB에서는 *한 줄의 plays 선언*.
...
*장면*. 17시 50분 34초. 임무 시작 2시간 50분 후. 진행률 *63%*.

관제 센터의 *적색 전화*가 울린다. 도청·119 통합관제실의 *우선순위 1 호출*. 분석가가 수화기를 들기 전부터 — 무슨 의미인지 안다. *다른 조난자*. 같은 산악 지역, *능선 너머 5.2km*. 부상자 한 명 — *심각한 외상, 의식 저하*. 헬리콥터 출동까지 *최소 40분*. 그동안 — *현재 비행 중인 군집*만이 도달 가능한 자리.

분석가의 손이 멈춘다. 첫 임무는 *60%를 넘었지만 종료까지 1시간 이상*. 두 조난자(첫 임무)는 *생명 위협 없음, 추위 노출 정도*. 새 신고의 부상자는 *생명 위협 임박*. 그러나 — *첫 임무를 중단하면 그 두 명이 발각되지 않는 자리에 더 오래 머문다*. 윤리적 결정이 *기술적 결정 위에 얹힌* 순간.

이번 사건은 — 시나리오 A·B·C와 *완전히 다른 모양*이다. 드론도, 링크도 손실되지 않았다. 모든 시스템이 *정상 작동*. 그런데 — *결정의 무게가 가장 크다*. *어느 임무에 더 많은 자원을 보낼 것인가*는 — *기계가 답할 수 없는 영역*이다. 
...
이 시나리오에서 — *시스템이 도울 수 있는 영역*이 정확히 무엇이고, *분석가만이 할 수 있는 영역*이 무엇인가가 *가장 또렷이 갈리는 순간*이다.
...
이것이 — *시나리오 D를 다루는 진짜 이유*다. 단순한 *재할당 예제*가 아니다. *의사결정의 온톨로지화*라는 *훨씬 큰 도메인*의 시작이다.

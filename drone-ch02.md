...
- *같은 자리에 두 사람* — 결혼의 두 배우자 모두 `spouse` role을 가짐. 이 *N항 관계의 다중성*.
...
도메인 사고가 *스키마로 충분히 표현되는 자리*와 *함수가 필요한 부분*가 갈라지는 첫 자리.
...
*John이 Mary와 결혼했다* — 누가 누구와 결혼했는가, 두 가지만 적으면 된다. 술어 논리로:
...
*John이 Mary와 1990년 6월 15일 결혼했다*. 시점이 추가됐다.

```
Marry(John, Mary, "1990-06-15")
```

세 자리로 늘어났다. 가능은 하지만 — *시점 없는 결혼*은 *Marry(John, Mary, ?)*? 어색하다.

**단계 3 — 정보가 더 추가될 때**

*John이 Mary와 1990년 6월 15일 파리에서 증인 Peter 앞에서 결혼했다*.

```
Marry(John, Mary, "1990-06-15", "Paris", Peter)
```

다섯 자리. 매번 정보가 추가될 때마다 *함수의 자리 수가 늘어난다*. 그리고 — *시점은 알지만 장소는 모르는 경우*는? *Marry(John, Mary, "1990-06-15", ?, ?)*? 매번 *unknown* 자리가 늘어남.

이게 *동사 표현의 본질적 어려움*이다. 함수의 자리 수가 *고정*되지 않는다.
...
이 책의 시간 처리(start/end date)는 *비교적 단순한 시간 모델*. 더 형식적인 영역에는 *시제 논리(temporal logic)*가 있다.
